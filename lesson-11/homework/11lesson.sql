...
1. **Return**: `OrderID`, `CustomerName`, `OrderDate`  
   ...
   **Tables Used**: `Orders`, `Customers`
   ...
   select OrderID, FirstName + ' ' + LastName as CustomerName, OrderDate
   from Orders O
...
4. **Return**: `CustomerName`, `OrderID`, `OrderDate`  
   ...
   **Tables Used**: `Customers`, `Orders`
   ...
   select FirstName + ' ' + LastName as CustomerName, OrderID,  OrderDate
   from Orders O
...
5. **Return**: `CustomerName`, `TotalOrders`  
   ...
   **Tables Used**: `Orders` , `Customers`
   ...
   select FirstName + ' ' + LastName as CustomerName,  Count(OrderID) as TotalOrders
   from Orders O
   join Customers C on O.CustomerID = C.CustomerID
...
7. **Return**: `CustomerName`, `MostRecentOrderDate`  
   ...
   **Tables Used**: `Customers`, `Orders`'
   ...
   select FirstName + ' ' + LastName as CustomerName,  Max(OrderDate) as MostRecentOrderDate
   from Orders O
   left join Customers C on O.CustomerID = C.CustomerID
   group by FirstName, LastName
   ;
...
8. **Return**: `CustomerName`,  `OrderTotal`  
   ...
   **Tables Used**: `Orders`, `Customers`
   ...
   select FirstName + ' ' + LastName as CustomerName, Sum(TotalAmount) as OrderTotal
   from Orders O
   left join Customers C on O.CustomerID = C.CustomerID
   group by FirstName, LastName
...
11. **Return**: `EmployeeName`, `DepartmentName`, `Salary`  
   ...
   from Employees E
   join Departments D on E.DepartmentID = D.DepartmentID
   where DepartmentName = 'HR' or Salary > 60000
   ;
...
13. **Return**: `EmployeeName`, `DepartmentName`, `HireDate`  
   ...
   from Employees E
   join Departments D on E.DepartmentID = D.DepartmentID
   where DepartmentName = 'HR' or  HireDate > '2020-12-31'
   ;
