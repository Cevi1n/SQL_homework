...
Select Min(Price), MAx(Price), Category
from Products
Group By Category
;
...
select p.Category, Sum(s.SaleAmount), Avg(s.SaleAmount)
from Sales s
...
select Max(Salary), Min(Salary), DepartmentName
from Employees
group by DepartmentName
;
...
select Sum(Salary), Avg(Salary), DepartmentName
from Employees
group by DepartmentName
