...
select concat_ws(' ', concat_ws('-', Employee_ID, First_Name), last_name)
from Employees;
...
select year1, Greatest(Max1, Max2, Max3)
from TestMax
...
WITH cte AS (
    SELECT 'tf56sd#%OqH' AS InputStr
)
SELECT
    InputStr,
    CONCAT_WS('', 
        CASE WHEN PATINDEX('%[A-Z]%', c) > 0 THEN c ELSE '' END) AS UppercaseLetters,
    CONCAT_WS('', 
        CASE WHEN PATINDEX('%[a-z]%', c) > 0 THEN c ELSE '' END) AS LowercaseLetters,
    CONCAT_WS('', 
        CASE WHEN PATINDEX('%[0-9]%', c) > 0 THEN c ELSE '' END) AS Digits,
    CONCAT_WS('', 
        CASE WHEN PATINDEX('%[^a-zA-Z0-9]%', c) > 0 THEN c ELSE '' END) AS Others
FROM cte
CROSS APPLY STRING_SPLIT(InputStr, '') AS s(c);
...
SELECT 
    equation,
    -- Baho qilish uchun dynamic SQL kerak
    -- Yoki t-SQL CLR yordamida ishlanadi
    -- SQL Server'da to'g'ridan-to'g'ri string'dan eval qilish yo'q
    'N/A in pure T-SQL' AS Result
FROM Equations;
