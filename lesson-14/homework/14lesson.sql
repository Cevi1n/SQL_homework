...
select Name , Left(name, charindex(',', Name) - 1) as Name, Right(Name, Len(Name) - charindex(',', Name))
...
SELECT 
  @str AS Original,
  TRANSLATE(@str, '0123456789', REPLICATE('X', 10)) AS ReplacedDigits;
...
select *, len(Vals) - len(replace(vals, '.', '')) as SpaceCount
from testDots
...
1. Write a SQL query to separate the integer values and the character values into two different columns.(rtcfvty34redt)
	...
	declare @str varchar(max) = 'rtcfvty34redt' 
	select @str as orginal, replace(TRANSLATE(@str, '0123456789', REPLICATE(' ', 10)), ' ', '') as Letters, 
	replace(TRANSLATE(@str, 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', REPLICATE(' ', 52)), ' ', '') as Integrs
...
SELECT 
    ...
    CONCAT(
        ...
        JSON_VALUE(j.value, '$[2]')
    ) AS SwappedVals
FROM (
    SELECT 
        Id,
        CAST('["' + REPLACE(Vals, ',', '","') + '"]' AS NVARCHAR(MAX)) AS jsonVals
    FROM MultipleVals
) t
CROSS APPLY (SELECT t.jsonVals AS value) j;
...
WeeklyTotals AS (
  SELECT 
      ...
      FinancialWeek,
      FinancialYear,
      SUM(ISNULL(SalesLocal, 0) + ISNULL(SalesRemote, 0)) AS WeeklySales
  FROM WeekPercentagePuzzle
...
SELECT 
    ...
    d.DailySales,
    w.WeeklySales,
    ROUND(100.0 * d.DailySales / NULLIF(w.WeeklySales, 0), 2) AS PercentageOfWeek
FROM DailySales d
JOIN WeeklyTotals w
    ON d.Area = w.Area AND d.FinancialWeek = w.FinancialWeek AND d.FinancialYear = w.FinancialYear
ORDER BY d.Area, d.Date;
