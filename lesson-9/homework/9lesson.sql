...
from Customers cu
join Orders ors on cu.CustomerID = ors.CustomerID
;
...
Select ProductName, *
from Orders ord
join Products pr on ord.ProductID = pr.ProductID
;
...
select *
from Payments pay
join Orders ord on ord.OrderID = pay.OrderID
;
...
select *
from Orders pay
join Products ord on ord.ProductID = pay.ProductID
where price > 100
;
...
select *
from Employees emp
join Departments de on emp.DepartmentID != emp.DepartmentID
;
...
select *
from Orders ord
join Products pr on TotalAmount > StocKQuantity
;
...
select *
from Customers cu
join Sales sa on cu.CustomerID = sa.CustomerID
...
select Name, CourseName
from Courses cu
join Enrollments enr on enr.CourseID = cu.CourseID
join Students st on enr.StudentID = st.StudentID
;
...
select Name, DepartmentName
from Employees emp
join Departments dep on emp.DepartmentID = emp.DepartmentID
;

18. Using Products, Categories table Show products where category is either 'Electronics'  'Furniture'.
...
SELECT p.PaymentID, p.OrderID, p.Amount AS PaidAmount,
       ...
       ABS(p.Amount - (o.Quantity * pr.Price)) AS Difference
FROM Payments p
